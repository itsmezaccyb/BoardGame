...
export interface PortType {
    ...
    /**
     * Size of the overlay icon as a fraction of the boat, default 0.22. A
     * single number applies everywhere; a map tunes it per style, since how
     * big an icon wants to be depends on the boat it is stamped onto.
     */
    logoScale?: number | Partial<Record<TileStyleId, number>>;
}
...
export function getPortLogoScale(id: PortTypeId, style: TileStyleId): number {
    const scale = getPortType(id)?.logoScale;
    if (typeof scale === 'number') return scale;
    return scale?.[style] ?? DEFAULT_LOGO_SCALE;
}
