...
// GET /api/chameleon/game-sessions?code=<session_code> - Get game session with players
export async function GET(request: NextRequest) {
  try {
    ...
    if (playersError) {
      console.error('❌ [Game Sessions API] Error fetching players:', playersError);
      return NextResponse.json({ error: 'Failed to fetch players' }, { status: 500 });
    }

    console.log(`✅ [Game Sessions API] Found session: ${code} with ${players?.length || 0} players`);
...
// POST /api/chameleon/game-sessions - Create new game session
...
// PUT /api/chameleon/game-sessions?code=<session_code> - Update game session
